...
public class RippleButton extends Button {
	...
	public RippleButton(Context context, AttributeSet attrs, int defStyle) {
		...
		init();
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RippleButton);
		mRippleColor = a.getColor(R.styleable.RippleButton_rippleColor, mRippleColor);
		mAlphaFactor = a.getFloat(R.styleable.RippleButton_alphaFactor, mAlphaFactor);
	...
	public boolean onTouchEvent(final MotionEvent event) {
		...
		if (event.getActionMasked() == MotionEvent.ACTION_DOWN && this.isEnabled() && mHover) {
			...
			mRadiusAnimator = ObjectAnimator.ofFloat(this, "radius", 0, dp(50)).setDuration(300);
...
//		super.setOnClickListener(l);
	...
	public void setRadius(final float radius) {
		mRadius = radius;
		if (mRadius > 0) {
			mRadialGradient = new RadialGradient(mDownX, mDownY, mRadius, adjustAlpha(mRippleColor, mAlphaFactor), mRippleColor,
